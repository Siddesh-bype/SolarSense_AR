...
def first_page(canvas, doc):
    ...
    canvas.drawCentredString(cx, page_h - 160, info.get("project_name", "SolarMitra"))
...
def later_pages(canvas, doc):
    ...
    canvas.drawString(15*mm, A4[1] - 10*mm, "SolarMitra")
...
def build_contact(data, styles, ai_paragraphs):
    ...
    elements.append(Paragraph(
        ...
        '<b>Thank you for choosing SolarMitra.</b>'
        '</font></para>',
        styles["BodyText2"],
    ))
    elements.append(Spacer(1, 4*mm))
...
def generate_report(data_path, output_path=None):
    ...
    if output_path is None:
        ...
        output_path = os.path.join(output_dir, f"SolarMitra_Report_{report_id}.pdf")
